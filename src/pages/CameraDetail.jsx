...
/**
 * Cerca la camera nei dati CRM di Corigliano come fallback
 * quando il WP REST API non ha il post type 'camera'.
 */
...
export default function CameraDetail() {
  ...
  // Prova prima WP REST API
  const { data: cameraWP, loading: loadingWP } = useWP(
    () => getCameraBySlug(slug),
    [slug]
  );

  // Fallback: dati CRM Corigliano
  const [cameraFallback, setCameraFallback] = useState(null);
  const [loadingFallback, setLoadingFallback] = useState(false);

  useEffect(() => {
    // Attiva il fallback solo se WP non ha trovato la camera
    if (!loadingWP && !cameraWP) {
      setLoadingFallback(true);
      getCorigliano()
        .then((data) => {
          const room = data?.rooms?.find(
            (r) => r.slug === slug || `camera-${r.id}` === slug
          );
          setCameraFallback(room ? normalizeCrmRoom(room) : null);
        })
        .catch(() => setCameraFallback(null))
        .finally(() => setLoadingFallback(false));
    }
  }, [loadingWP, cameraWP, slug]);

  if (loadingWP || loadingFallback) return <Loader />;

  const camera = cameraWP || cameraFallback;

  if (!camera) {
    return (
      <div className="section-padding text-center">
        <h2>Camera non trovata</h2>
        <Link to="/strutture/corigliano" className="btn-bnb mt-3">
          ← Torna a Corigliano
        </Link>
      </div>
    );
      ...
      <div className="container">
        ...
        <div className="row g-5">
          <div className="col-lg-7 fade-in">
            {camera.featuredImage && (
              <img
                ...
                alt={camera.featuredImage.alt}
                className="w-100 img-bnb"
          ...
          <div className="col-lg-5 fade-in fade-in-d1">
            ...
            {camera.content && (
              <div
                className="wp-content mb-4"
                dangerouslySetInnerHTML={{ __html: camera.content }}
              />
            )}

            <div className="d-flex gap-3 flex-wrap">
              <a
                href={camera.bookingUrl || BOOKING_URL}
                target="_blank"
