...
export default function BookingWidget() {
  ...
  const handleCheckinBlur = () => {
    ...
    if (isRangeBlocked(form.checkin, form.checkout, blocked)) {
      setError("Le date selezionate non sono disponibili. Scegli un altro periodo.");
    } else {
      setError(null);
    }
  };
  ...
  if (step === "struttura") {
    return (
      <div style={card}>
        <h4 style={{ fontFamily: "var(--font-display)", marginBottom: "1.5rem" }}>
          ...
          {STRUTTURE.map((s) => (
            <div className="col-md-6" key={s.id}>
              <button
                ...
                onClick={() => {
                  setStruttura(s.id);
                  if (s.id === "sternatia") { setUnit("sternatia"); setStep("date"); }
                  else { setStep("camera"); }
                }}
                style={{
                  width: "100%", padding: "1.5rem", textAlign: "left",
  ...
  if (step === "camera") {
    return (
      <div style={card}>
        <button type="button" onClick={() => setStep("struttura")}
          style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-muted)", fontSize: "0.85rem", marginBottom: "1rem", padding: 0 }}>
          ← Torna indietro
        </button>
        <h4 style={{ fontFamily: "var(--font-display)", marginBottom: "1.5rem" }}>
        ...
        <div className="row g-3">
          {CAMERE.map((c) => (
            <div className="col-md-6" key={c.id}>
              <button type="button"
                onClick={() => { setUnit(c.id); setStep("date"); }}
                style={{
                  width: "100%", padding: "1.5rem", textAlign: "center",
                  background: "var(--color-bg)", border: "2px solid var(--color-border)",
                  borderRadius: "var(--radius-lg)", cursor: "pointer",
                  fontFamily: "var(--font-display)", fontSize: "1.1rem", transition: "border-color 0.2s",
                }}
  ...
  if (step === "date") {
    ...
    return (
      <div style={card}>
        <button type="button"
          onClick={() => setStep(struttura === "corigliano" ? "camera" : "struttura")}
          style={{ background: "none", border: "none", cursor: "pointer", color: "var(--color-muted)", fontSize: "0.85rem", marginBottom: "1rem", padding: 0 }}>
          ← Torna indietro
        </button>
        ...
        <p style={{ fontSize: "0.85rem", color: "var(--color-muted)", marginBottom: "1.5rem" }}>
          {loadingCalendar ? "Caricamento disponibilità…" : "Seleziona il periodo di soggiorno."}
        </p>

        <div className="row g-3">
          <div className="col-md-6">
            <label style={labelStyle}>Check-in</label>
            <input type="date" name="checkin" value={form.checkin} min={TODAY}
              onChange={handleCheckin} onBlur={handleCheckinBlur} style={inputStyle} />
          </div>
          <div className="col-md-6">
            <label style={labelStyle}>Check-out</label>
            <input type="date" name="checkout" value={form.checkout} min={form.checkin || TODAY}
              onChange={handleCheckout} disabled={!form.checkin}
        ...
        {error && (
          <p style={{ color: "var(--color-danger, #dc3545)", fontSize: "0.88rem", marginTop: "1rem" }}>
            ⚠ {error}
          </p>
        )}
        ...
        {nights > 0 && !error && (
          <p style={{ marginTop: "1rem", fontSize: "0.9rem", color: "var(--color-muted)" }}>
            📅 {fmt(form.checkin)} → {fmt(form.checkout)} · <strong>{nights} nott{nights === 1 ? "e" : "i"}</strong>
          </p>
        )}

        {nights > 0 && !error && (
          <div style={{ marginTop: "0.75rem", padding: "1rem 1.25rem", borderRadius: "var(--radius)", background: "var(--color-bg-warm, #faf7f4)", border: "1px solid var(--color-border)" }}>
            {loadingPricing ? (
              <p style={{ margin: 0, fontSize: "0.85rem", color: "var(--color-muted)" }}>Calcolo prezzo…</p>
  ...
  if (step === "dati") {
    ...
    return (
      <div style={card}>
        ...
        <div style={{ background: "var(--color-bg-warm, #faf7f4)", borderRadius: "var(--radius)", padding: "1rem 1.25rem", marginBottom: "1.5rem", fontSize: "0.88rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: "0.5rem" }}>
            <div>
              <strong>{UNIT_LABELS[unit]}</strong><br />
              {fmt(form.checkin)} → {fmt(form.checkout)} · {nights} nott{nights === 1 ? "e" : "i"} · {form.ospiti} ospiti
            </div>
            {pricing && !pricing.errori?.length && (
        ...
        <div className="row g-3">
          <div className="col-md-6">
            ...
            <input type="text" name="nome" value={form.nome} onChange={handleChange}
              required placeholder="Il tuo nome" style={inputStyle} />
          </div>
          <div className="col-md-6">
            <label style={labelStyle}>Email *</label>
            <input type="email" name="email" value={form.email} onChange={handleChange}
              required placeholder="email@esempio.com" style={inputStyle} />
          </div>
          <div className="col-md-6">
            <label style={labelStyle}>Telefono</label>
            <input type="tel" name="telefono" value={form.telefono} onChange={handleChange}
              placeholder="+39..." style={inputStyle} />
          </div>
          <div className="col-12">
            <label style={labelStyle}>Messaggio (opzionale)</label>
            <textarea name="messaggio" value={form.messaggio} onChange={handleChange}
              rows="3" placeholder="Richieste particolari, orario di arrivo previsto…"
