...
// ── Galleria ──
// Aggrega tutte le foto delle gallery degli alloggi (Sternatia + camere
// Corigliano + spa) usando l'endpoint /strutture già esistente.
...
export async function getGalleryImages() {
  ...
  const pushGallery = (gallery) => {
    ...
    for (const item of gallery) {
      ...
      images.push({
        url,
        alt: (item && typeof item === 'object' && item.alt) || '',
      });
    }
  };

  // Sternatia (casa intera)
  pushGallery(raw.sternatia?.gallery);
  // Corigliano: camere + spa (Menima)
  (raw.corigliano?.rooms || []).forEach((room) => pushGallery(room.gallery));
  pushGallery(raw.corigliano?.spa?.gallery);
