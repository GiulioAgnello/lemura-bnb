...
export const metadata = {
  ...
  openGraph: {
    title: 'Recensioni 5 stelle | Le Mura degli Angeli B&B Salento',
    description: 'Cosa dicono i nostri ospiti del B&B Le Mura degli Angeli a Corigliano d\'Otranto e Sternatia, nel Salento.',
  },
};
...
const PLACEHOLDER = [
  ...
  { id: 6, nome: 'Francesca', provenienza: 'Bologna', stelle: 5, testo: "Ogni dettaglio è curato con amore. La struttura è un gioiello di architettura salentina restaurata con gusto. Colazione eccezionale.", data_soggiorno: 'Aprile 2025', piattaforma: 'Booking.com' },
];

export default async function Recensioni() {
  const recensioniWP = await getRecensioni();
              ...
              <div className="col-md-6 col-lg-4 fade-in" key={r.id} style={{ animationDelay: `${i * 0.07}s` }}>
                <div className="testimonial-card h-100 d-flex flex-column">
                  <div className="d-flex justify-content-between align-items-center mb-2 mt-2">
                    ...
                    {r.piattaforma && <small className="text-muted" style={{ fontSize: '0.75rem' }}>{r.piattaforma}</small>}
                  </div>
                  <p className="flex-grow-1" style={{ fontStyle: 'italic', fontSize: '0.95rem', color: 'var(--color-text)', lineHeight: 1.7 }}>{r.testo}</p>
